...
for i=1:n_hlayer  %vary number of hidden layer neurons from 1 to 100
   ...
   tic 
    ...
    net = feedforwardnet([hiddenLayerSize1,hiddenLayerSize2,hiddenLayerSize3],trainFcn);
    ...
    c1 = strcat(a1,'-',a2,'-',a3);
    c2 = strcat(a1,a2,a3);
    layer(k) = str2double(c2);
       
    maetr(k) = mean(abs(output_tr-t(tr.trainInd)));
    maete(k) = mean(abs(output_te-t(tr.testInd)));
    
    mretr(k) = mean(abs(output_tr-t(tr.trainInd))/output_tr);
    mrete(k) = mean(abs(output_te-t(tr.testInd))/output_te);
        
    
    msetr(k) = mean((output_tr-t(tr.trainInd)).^2);
    msete(k) = mean((output_te-t(tr.testInd)).^2);
        
    rmsetr(k) = sqrt(mean((output_tr-t(tr.trainInd)).^2)); %RMSE for 70% random trainig data
    rmsete(k) = sqrt(mean((output_te-t(tr.testInd)).^2)); %RMSE for 15% random test data
        
    R_tr(k) = regression(t(tr.trainInd), output_tr);
    R_te(k) = regression(t(tr.testInd), output_te);
